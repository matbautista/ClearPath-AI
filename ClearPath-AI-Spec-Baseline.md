...
## Product Specification v9 — BASELINE (CLOSED)

**Status: Baselined and closed.** This version is the frozen reference for implementation (database schema, ER diagram, build work). Nine rounds of review across v1→v9 have resolved every cross-section contradiction found; no open decisions remain. Further changes should be made as a new version (v10) rather than edited in place, so this baseline stays a stable point to build against and compare future changes to. Supersedes v8.
...
## Changelog: v8 → v9 (final closing pass)

Two small edge cases left over from formalizing `Goal` in v8 — both about what happens when a balance moves *backward* after a threshold was already evaluated, which the rest of the doc has consistently handled by preferring live-derived state over cached/one-way flags:

1. **Goal completion wasn't reversible.** v8 defined completion as "Status → Completed automatically when balance reaches zero/Target Amount" but never said what happens if the balance later moves again — e.g. the payment that completed a Debt Payoff goal gets voided (2.6), or a paid-off debt account takes a new charge/disbursement. Fixed: `Status` is continuously derived from live balance state, not a one-way ratchet — it reverts Completed → Active automatically if a linked balance becomes nonzero again. This is the same "derived, not cached" rule already applied to `Available Balance` (2.1) elsewhere in this spec, just applied to Goal Status too.
2. **Allocation over-commit validation only applies going forward.** §3.10 blocks a *new* allocation that would push a account's linked-goal total past 100% of its current balance, but doesn't retroactively re-check when the balance itself later drops (e.g. ordinary spending reduces a savings account after two goals already claimed 100% of the old, higher balance). Clarified: consistent with the immutable-ledger model (2.6), past transactions aren't revisited — a resulting over-100% allocation is surfaced visibly (progress bars reflect the shortfall) rather than silently capped or blocked.

These were the only findings this round, both minor and both resolved by extending a rule the spec already uses elsewhere rather than introducing a new one — a good signal this is a reasonable point to stop the review loop and move to schema/ER-diagram work.
...
| Beginning Balance | For a **pre-existing** account being onboarded into ClearPath AI — its balance as of setup, with no matching transaction history behind it. For an account **opened after go-live**, this is simply 0 and its balance builds up entirely from ordinary transactions — **except a new Loan**, which starts with a nonzero amount owed that must come from somewhere: that's what the Loan Disbursement transaction (2.5) is for (a new Credit Card needs no equivalent special transaction — it just starts at Card Balance = $0 and accrues normally through Card Purchases). |
...
| Indicator | Debit or Credit — see "Indicator direction" below for exactly what increases/decreases means per account type. |
| Source Account | Required. The one account **this row** affects (see "Two-leg transactions are two rows" below). |
| Destination Account | Populated for two-leg transactions (transfers, buy/sell, loan/card payments, disbursements) — mirrors the *other* linked row's Source Account, for display/audit convenience. Null for genuinely single-leg transactions (income, dividends, utility/tax payments, card purchases). |
| Transaction Type | Mechanical type — see 2.3 |
| Spending Category | Applies to every Debit-indicator single-leg transaction, **and to both linked rows of any two-leg transaction** (same value on each row, mirroring the underlying event) — see 2.4. Loan Disbursement is the one two-leg type that carries no category at all (borrowed principal is neither spending nor income). |
| Income Category | Applies **only** to genuinely single-leg external inflows — Income Received (denormalized from the linked Income Source's Income Category, 3.8) and Dividend/Interest Received (chosen directly, e.g. "Dividends"/"Interests"). Never used on a two-leg transaction's Credit-side leg (there's no value in this set for "this is just your own money"). |
...
| Linked Transaction ID | Self-reference to the paired row for two-leg transactions (transfers, buy/sell, payments, disbursements). See 2.5. |
| Status | Posted, Voided, or Pending Confirmation (auto-drafted recurring items awaiting user confirmation — see 2.7) |

**Two-leg transactions are two rows, not one row with two account fields.** A transfer, buy/sell, loan/card payment, or disbursement posts as **two independent Transaction rows**, linked via `Linked Transaction ID` — confirmed by 3.3 ("posts two linked entries"), 2.6 ("voiding either leg"), and the Additional Fees rule (2.5), where each leg genuinely has its own Amount/Fee values. Each row's own `Source Account` is the account *that row* mutates; its `Destination Account` just mirrors the paired row's `Source Account` for display. A single-leg transaction (Destination Account null) is the only case where one row is the whole story.

**Indicator direction.** `Indicator` always describes whether *that row's own account* moved in the user's favor (Credit) or against it (Debit) — this is one consistent rule across all account types, but "in the user's favor" means something different for assets vs. liabilities:
- Cash, Bank, Investment (assets — tracked as `Current Balance`, money/value held): Credit = balance increases, Debit = balance decreases.
- Loan, Credit Card (liabilities — tracked as `Current Balance`/`Card Balance`, amount owed): Credit = amount owed decreases (a payment), Debit = amount owed increases (a new charge or disbursement).

This is why a Card Purchase is Debit (you owe more) and a Card Payment's card-side leg is Credit (you owe less) even though both are "the credit card account" — and it's why every two-leg transaction naturally has one Debit leg and one Credit leg, except Loan Disbursement, where cash increases (Credit) *and* the amount owed increases (Debit on the loan leg) — those are opposite-direction leg types too, just both "increases" in absolute terms since one is an asset and one is a liability.

**Category totals count a linked pair once, not twice.** Because both legs of a two-leg transaction can carry the same Spending Category (above), any total for that category (Category Trend Detection 3.11a, Savings Rate 3.1, spending reports/exports 4.4) is computed from the **Debit leg's Amount only** (or, for `Debt Payment`, the Debit leg's Interest Portion only, per 2.4a) — never both legs added together. This generalizes the Debt-Payment-Interest-Portion-only rule already established in 2.4a into one system-wide rule, so a single transfer or purchase is never double-counted as though it happened twice.
...
`Internal Transfer` and `Savings/Investment Transfer` are both **excluded from Category Trend Detection (3.11a) and Savings Rate (3.1)**: moving your own money between your own accounts, or into savings/investments, is not spending — neither should ever trigger a false money-pit flag or drag down the savings figure. They stay distinct from each other (and from `Other`) purely so reports can still show *where* money that isn't being spent is going.
...
Every transaction has a **source account** and, where money moves between two accounts the user owns or owes, a **destination account** on the paired row (2.2). The system always updates both sides. This single rule replaces the need to separately document balance-update behavior for every module — see 2.2 for the two-row structure, the Debit/Credit direction convention, and the category-mirroring/counted-once rule that apply to every bullet below:

- Bank-to-bank transfer → debit source bank, credit destination bank. Both legs: Spending Category `Internal Transfer` (or a more specific one the user picks, e.g. moving cash to a bills-paying account).
- Cash withdrawal from bank → debit bank, credit cash on hand. Both legs: `Internal Transfer`.
- Credit card bill payment → debit source (cash/bank), credit card balance (reduce Card Balance, increase Available Balance). Both legs: `Debt Payment` (only the Interest Portion counts toward the category total, 2.4a).
- Loan payment → debit source (cash/bank), credit loan balance (reduce Current Balance). Both legs: `Debt Payment` (Interest Portion only counts, 2.4a).
- Loan disbursement → credit destination (cash/bank, balance increases by disbursed amount); debit the Loan account (Current Balance/amount owed increases by the same amount) — the mirror of Loan payment. This is how a loan originated after go-live gets its opening balance entirely from transaction history rather than a hand-set field (see `Beginning Balance`, 2.1). No Spending/Income Category (2.2) — borrowed principal is neither.
- Investment purchase → debit source (cash/bank), credit investment account. Both legs: `Savings/Investment Transfer`.
- Investment sale → debit investment account, credit destination (cash/bank). Both legs: `Savings/Investment Transfer` (2.4a — v1 doesn't compute realized gains, so this is never tagged as income).
...
- Utility/tax payment → debit source (cash/bank), single-leg expense (per 2.2's field semantics — Spending Category applies, e.g. `Utilities` or `Taxes/Fees`).
...
`Goal` and its account linkage are formalized here, matching how every other entity in this section is specified:
...
| Total Monthly Payment Budget | Debt Payoff only, when more than one debt account is linked — feeds the Multi-debt allocation rolldown (3.10) |
| Status | Active, Completed, or Abandoned. Active/Completed are **continuously derived from live balance state, not a one-way ratchet** (v9 fix): if a linked account's balance becomes nonzero again after Completed is reached — e.g. the completing payment gets voided (2.6), or a new charge/disbursement posts to an otherwise-paid-off debt — Status automatically reverts to Active. This mirrors the same "derived, not cached" rule already used for `Available Balance` (2.1); Completed is a read of current balances, never a stored flag that can drift from them. Abandoned is the only user-initiated state (e.g., after a linked account closes and the user chooses to retire the goal instead of re-linking, 2.1) and is not automatically reverted by balance changes. |

**Goal Account Link** (join entity for "Linked Account(s)" and "Allocation", 3.10)
| Field | Notes |
|---|---|
| Goal ID | |
| Account ID | |
| Allocation Type | Fixed Amount or Percentage |
| Allocation Value | The fixed amount or percentage this goal claims of the account's balance |
...
- **Savings Rate**: deterministic, no AI required — `(Income total − Expense total) / Income total` over a selectable trailing period (e.g. 30/90 days), computed straight from Posted transactions:
  - **Income total** = all Credit-indicator single-leg transactions (Income Received + Dividend/Interest Received). Loan Disbursement is excluded — borrowed money isn't income.
  - **Expense total** = the same Spending-Category totals Category Trend Detection (3.11a) computes — which excludes `Internal Transfer` and `Savings/Investment Transfer` (2.4), counts only the Interest Portion of debt payments (2.4a), and counts each two-leg transaction once, not per-leg (2.2). One shared definition of "what counts as spending," not a second one maintained separately here.
...
- Create transactions: deposits, withdrawals, incoming/outgoing transfers (including own-account-to-own-account, which posts two linked entries per rule 2.5, both tagged `Internal Transfer` per 2.4), and payments made from the account. e-wallet activity (GCash, Maya, etc.) is recorded here using the `eCash Transfer` / `eCash Payment` transaction types (2.3) against a Bank-type account set up for that wallet.
- List recent (within the month) transactions.

### 3.4 Investments
- Set up account (Broker Name, Description, Account Number, Account Name, Beginning Balance, Current Balance).
- Create transactions: buy, sell (both legs tagged `Savings/Investment Transfer` per 2.4/2.4a/2.5), dividend/interest received (single-leg, Income Category).
- List recent (within the month) transactions.
...
- Create transactions: purchases (single-leg, Debit — increase Card Balance, decrease Available Balance, Spending Category applies) and bill payments (two-leg, linked to a source account per rule 2.5, both legs `Debt Payment`).
- List recent (within the month) transactions.
...
Data model: `Goal` and `Goal Account Link` (2.8).

- **Goal Type**: Debt Payoff, Emergency Fund, Savings Target, Investment Target.
- **Target Amount**, **Target Date**.
- **Linked Account(s)**: which account(s) count toward this goal, via `Goal Account Link` (2.8). An account may be linked to more than one goal (e.g., one bank account backing both an Emergency Fund and a Vacation goal); in that case, each goal's link also stores an **Allocation** — either a fixed amount or a percentage of the account's balance — so progress bars split the same balance rather than each independently claiming the full amount. **Validation**: the system tracks total allocation per account across all its linked goals and blocks saving a new/edited allocation that would push the sum past 100% of the account's Current Balance, with a clear error naming the conflicting goal(s). This check runs at allocation-edit time only — it does not retroactively re-validate when the account's balance later drops due to ordinary spending (v9 fix: consistent with 2.6's immutable-ledger model, past transactions aren't revisited); if that pushes linked allocations past 100% of the new, lower balance, progress bars simply reflect the shortfall rather than the system silently capping or blocking historical data. If a linked account is closed (2.1), the goal is flagged for the user to re-link or retire (retiring sets Status = Abandoned, 2.8).
...
- **Inputs**: account balances, transaction history, spending categories, active goals (Status = Active, 2.8), Money-Pit flags (3.11a), Savings Rate (3.1).
- **Outputs**:
  - Written summary of current financial standing (income vs. expenses, debt-to-income, and savings rate — reusing the deterministic figure from 3.1 rather than recomputing it).
...
- **Category trend detection**: flags a Spending Category (2.4) whose rolling monthly total has grown significantly versus its own trailing average (e.g., "Groceries" up 40% over 3 months — this is how something like excessive potato purchases would surface, as a grocery-category trend) — surfaces the trend, not a judgment about whether it's a problem. `Internal Transfer` and `Savings/Investment Transfer` (2.4) are both excluded from this detector — moving your own money between your own accounts, or into savings/investments, isn't spending and must never generate a money-pit flag. Category totals count each two-leg transaction once, not per-leg (2.2).
...
| Goal allocation over-commit | Blocked at edit-time — total allocation per account across goals can't exceed 100%; not retroactively enforced against later balance drift | 3.10 |
...
| Money-pit / spending-pattern detection | Local (non-API) category-trend and recurring-charge clustering; excludes Internal Transfer and Savings/Investment Transfer | 3.11a |
...
| Internal (own-account) transfers | `Internal Transfer` spending category, excluded from Category Trend Detection and Savings Rate | 2.4/3.11a/3.1 |
| Single-leg transaction field semantics | `Source Account` = the one internal account touched; `Indicator` reflects that account's own balance/amount-owed direction | 2.2 |
| Loan Disbursement | Two-leg like Loan Payment: credits destination (cash/bank), simultaneously debits the Loan account (Current Balance/amount owed increases). Beginning Balance is for pre-existing loans only; new loans use Disbursement. No Spending/Income Category | 2.1/2.2/2.5 |
...
| Spending vs. Income category split | `Income Category` is reserved for genuinely single-leg external inflows (Income Received, Dividend/Interest Received) only; every two-leg transaction tags both linked rows with the same `Spending Category` instead | 2.2 |
| Savings/Investment Transfer exclusion | Consistently excluded from Category Trend Detection and Savings Rate everywhere it's stated (2.4, 3.1, 3.11a) | 2.4/3.1/3.11a |
| Two-leg category double-counting | Category totals count a linked pair once (off the Debit leg), generalizing the existing Debt-Payment-Interest-Portion-only rule system-wide | 2.2 |
| Two-row transaction structure | Made explicit: two-leg transactions are two independent, linked Transaction rows, not one row with both Source and Destination populated | 2.2 |
| Indicator direction per account type | Clarified: Debit/Credit reflects that leg's own account moving against/in favor of the user — decrease/increase for assets, increase/decrease amount owed for liabilities | 2.2 |
| Goal data model | Formalized as `Goal` + `Goal Account Link` entities, matching Account/Transaction/Settings/Recurring Rule | 2.8 |
| Goal completion trigger | Defined strictly by balance (linked debts at zero, or linked savings/investment balance at Target Amount) — never by Account Status, to avoid the closed-account-with-balance bug this project has already been burned by once. Multi-debt rolldown reuses the same "paid off" check | 2.8/3.10 |
| Goal completion reversibility | Status is continuously derived from live balance state, not a one-way ratchet — reverts Completed → Active if a linked balance becomes nonzero again (voided transaction, new charge/disbursement) | 2.8 (v9) |
| Allocation over-commit vs. balance drift | Validated at edit-time only; a later balance drop that pushes allocations past 100% is surfaced visibly (progress bars reflect the shortfall), not silently capped or blocked, consistent with the immutable-ledger model | 3.10 (v9) |

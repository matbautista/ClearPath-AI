...
## Product Specification v7 — BASELINE

**Status: Baselined.** This version is frozen as the reference for implementation (database schema, ER diagram, build work). Further changes should be made as a new version (v8) rather than edited in place, so this baseline stays a stable point to build against and compare future changes to. Supersedes v6.
...
Checked v6's own new additions (the Spending/Income Category split, the Savings Rate formula) against each other and against the sections they depend on. Found:

1. **`Savings/Investment Transfer` exclusion was inconsistent across three sections — and the gap would have made Savings Rate punish saving.** §2.4 and §3.11a only ever explicitly excluded `Internal Transfer` from spending/trend-detection; §3.1's Savings Rate formula (added in v5/v6) claimed both `Internal Transfer` *and* `Savings/Investment Transfer` were already excluded — which wasn't true anywhere else in the doc. As written, moving money into savings or investments would have counted as an "expense," so the more a user actually saved, the lower their Savings Rate would appear. Fixed by excluding both categories, consistently, everywhere the exclusion is stated.
2. **v6's "Credit-indicator transactions get an Income Category" rule was too broad and contradicted §3.3's own text.** §3.3 says an internal transfer "posts two linked entries... tagged `Internal Transfer`" — both legs share that Spending Category. But v6's rule would have forced the Credit-indicator leg (the receiving account) into `Income Category` instead, which has no value for "this is just your own money." Fixed: `Income Category` is now reserved for genuinely single-leg external inflows only (Income Received, Dividend/Interest Received — i.e., Destination Account is null); every two-leg transaction (transfers, buy/sell, loan/card payments, disbursement) tags **both** linked legs with the same `Spending Category`, regardless of which leg is Debit or Credit. Loan Disbursement keeps its existing exception (neither category — borrowed principal is neither spending nor income).
3. **Category totals could have double-counted two-leg transactions.** If both legs of a transfer/buy-sell/payment carry the same category (fix #2), naively summing "all transactions in category X" would count each transfer twice. §2.4a already had the right instinct for this (only the Interest Portion of a debt payment counts toward the `Debt Payment` total, not the full Amount) — generalized that into one explicit system-wide rule: a linked pair's economic size is counted **once** per category total, off the Debit leg, not once per leg.
4. **The two-row transaction model was implied but never stated as a rule.** Confirmed from scattered existing clues (§3.3 "posts two linked entries," §2.6 "voiding either leg," the fee rule where each leg's own Amount/Fee genuinely differs) that every two-leg transaction is **two independent, linked Transaction rows** — not one row with both Source and Destination Account populated. Stated explicitly now, given how much confusion this exact area has caused across three prior review rounds.
5. **Clarified (not changed) what Debit/Credit means per account type**, since this was genuinely ambiguous on a re-read even though §2.5's existing bullets turned out to already be correct: for Cash/Bank/Investment, Debit = balance held decreases, Credit = increases; for Loan/Credit Card, Debit = amount owed increases, Credit = amount owed decreases (paying down debt is a Credit on that leg, matching what §2.5 already said — the ambiguity was in v5/v6's own explanatory paragraph, not in §2.5 itself).
...
- **Inputs**: account balances, transaction history, spending categories, active goals, Money-Pit flags (3.11a), Savings Rate (3.1).
- **Outputs**:
...
| Spending vs. Income category split | `Income Category` is reserved for genuinely single-leg external inflows (Income Received, Dividend/Interest Received) only; every two-leg transaction tags both linked rows with the same `Spending Category` instead (v7 fix — v6's rule was too broad and contradicted 3.3) | 2.2 (v7) |
| Savings/Investment Transfer exclusion | Now consistently excluded from Category Trend Detection and Savings Rate everywhere it's stated (2.4, 3.1, 3.11a) — v6 had this right only in 3.1, contradicting 2.4/3.11a | 2.4/3.1/3.11a (v7) |
| Two-leg category double-counting | Category totals count a linked pair once (off the Debit leg), generalizing the existing Debt-Payment-Interest-Portion-only rule system-wide | 2.2 (v7) |
| Two-row transaction structure | Made explicit: two-leg transactions are two independent, linked Transaction rows, not one row with both Source and Destination populated | 2.2 (v7) |
| Indicator direction per account type | Clarified (not changed): Debit/Credit reflects that leg's own account moving against/in favor of the user — decrease/increase for assets, increase/decrease amount owed for liabilities. Resolves an ambiguity in v5/v6's explanatory text; §2.5's bullets were already correct | 2.2 (v7) |

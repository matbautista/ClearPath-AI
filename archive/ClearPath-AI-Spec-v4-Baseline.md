...
## Product Specification v4 — BASELINE

**Status: Baselined.** This version is frozen as the reference for implementation (database schema, ER diagram, build work). Further changes should be made as a new version (v5) rather than edited in place, so this baseline stays a stable point to build against and compare future changes to. Supersedes v3.
...
Four cross-section issues found in review of v3, all fixed below:

1. **Credit card balance modeling (2.1, 2.4b, 3.6)**: v3's unified `Account` model had only one `Current Balance` field, but a credit card needs two numbers that move in opposite directions on the same purchase (amount owed vs. spending room) — one field can't represent both. Fixed by adding a dedicated `Card Balance` field and making `Available Balance` a **derived** value (Credit Limit − Card Balance), not a second stored field — derived state should be computed from source data, not kept as a second mutable copy that can drift.
2. **Reminder lead time override had no field (2.0, 2.1, 2.7)**: v3's Settings section promised a per-item lead-time override for Loans/Cards/Utilities/Income/Tax records, but no entity in the data model actually had that field. Added `Reminder Lead Time` to `Account` (Loan/Credit Card) and `Recurring Rule` (Utility/Income/Tax), following the same Account-vs-Recurring-Rule split already used for due-date sourcing (3.1).
3. **Closed accounts with a nonzero balance could vanish from Net Worth (2.1, 3.1)**: v3 excluded closed accounts from "Dashboard totals" with no balance-zero requirement to close, so a card/loan closed while still owed money would silently disappear from Net Worth and the upcoming-dues breakdown. Fixed: closed accounts are excluded from the per-type summary widgets, but remain in Net Worth and upcoming dues until their balance reaches zero.
4. **Loan/Credit Card module field names drifted from the unified model (3.5, 3.6)**: leftover pre-unification names (`Loan Number`, `Loan Balance`) didn't match the unified `Account` table's `Account Number` / `Current Balance`. Renamed for consistency. Also split the `Cut-off Date / Due Date` table row — loans have a Due Date but no billing-cycle Cut-off Date; only Credit Card and Utility do.

Also added an `Internal Transfer` spending category (2.4) so same-owner account-to-account transfers don't land in an expense category and falsely trigger money-pit flags (3.11a).
...
| Beginning Balance | |
| Current Balance | Cash, Bank, Investment, and Loan only — derived from transaction history (see Valuation Method for Investment accounts). Credit Card accounts use `Card Balance` / `Available Balance` below instead (v4 fix — see 2.4b). |
| Card Balance | Credit Card only (v4 fix). Amount owed — derived from transaction history the same way `Current Balance` is for other types: increases on Card Purchase, decreases by the Principal Portion (2.2) of a Card Payment. This is the credit card's "Current Balance" equivalent. |
| Available Balance | Credit Card only (v4 fix). **Derived, not stored**: `Credit Limit − Card Balance`, recomputed live and cached only for display (2.4b). Never independently edited or drifted from Card Balance. |
...
| Cut-off Date | Credit Card, Utility only (v4 fix — loans have no billing-cycle cutoff in this spec, only a due date). |
| Reminder Lead Time | Loan and Credit Card only (v4 fix). Optional override of Settings' Default Reminder Lead Time (2.0); when absent, the global default applies. Utility/Income Source/Tax records use the equivalent field on their `Recurring Rule` (2.7) instead. |
| Status | Active or Closed/Archived. Closed accounts keep their transaction history and cannot be linked to new transactions or new Goals — a Goal referencing a closed account is flagged for the user to re-link or retire (3.10). **v4 fix**: closing an account has no balance-zero requirement, so status and balance are tracked independently. Closed accounts are excluded from the per-type Dashboard summary widgets (3.1) unconditionally, but remain included in Net Worth and the upcoming-dues breakdown (3.1) for as long as their balance is nonzero — a closed credit card or loan that still carries a balance is a real liability and must not silently disappear from either. Once a closed account's balance reaches zero, it drops out of Net Worth/dues too, leaving only its transaction history and export trail. |
...
| Reminder Lead Time | Optional override of Settings' Default Reminder Lead Time (2.0) (v4 fix); when absent, the global default applies. Loan/Credit Card equivalents live on `Account` instead (2.1), since those aren't backed by a Recurring Rule. |
...
- **Net Worth** (Assets − Liabilities, new). **v4 fix**: includes closed accounts for as long as their balance is nonzero (2.1) — a closed card/loan that's still owed money doesn't disappear from Net Worth just because it's closed.
...
- Create transactions: deposits, withdrawals, incoming/outgoing transfers (including own-account-to-own-account, which posts two linked entries per rule 2.5, tagged `Internal Transfer` per 2.4), and payments made from the account.
- List recent (within the month) transactions.
...
### 3.5 Loans
- Set up loan (Lender Name, Description, Account Number, Loan Amount, Current Balance, Loan Term, Interest Rate, Minimum Payment, Due Date, optional Reminder Lead Time override) — field names aligned with the unified Account table (2.1) (v4 fix: was `Loan Number` / `Loan Balance`).
...
- **Category trend detection**: flags a Spending Category (2.4) whose rolling monthly total has grown significantly versus its own trailing average (e.g., "Groceries" up 40% over 3 months) — surfaces the trend, not a judgment about whether it's a problem. **v4 fix**: the `Internal Transfer` category (2.4) is excluded from this detector — moving your own money between your own accounts isn't spending and must never generate a money-pit flag.
...
### 4.5 Notifications
Upcoming dues (loans, cards, utilities, taxes) and pending recurring transactions (2.7/3.7) need a delivery channel, not just a dashboard listing. v1 scope: in-app banner/badge plus optional email reminder. Lead time uses the global Default Reminder Lead Time (2.0) unless a specific item overrides it via `Account.Reminder Lead Time` (Loan/Credit Card, 2.1) or `Recurring Rule.Reminder Lead Time` (Utility/Income/Tax, 2.7) (v4 fix — fields now exist to back this). Since a self-hosted instance has no built-in mail server, email delivery requires the user to supply their own SMTP credentials in Settings (2.0). Push notifications are a future consideration pending whether ClearPath AI ships a mobile client.
...
| Reminder lead time | Global default in Settings, overridable per item via `Account.Reminder Lead Time` (Loan/Card) or `Recurring Rule.Reminder Lead Time` (Utility/Income/Tax) — fields added in v4 | 2.0/2.1/2.7/4.5 |
...
| Money-pit / spending-pattern detection | Added — local (non-API) category-trend and recurring-charge clustering; explicitly can't detect "underutilization," only cost/frequency; excludes Internal Transfer (v4) | 3.11a |
| Credit card balance modeling | `Card Balance` is the stored/authoritative field; `Available Balance` is derived (Credit Limit − Card Balance), never independently stored | 2.1/2.4b (v4) |
| Closed account with nonzero balance | Excluded from per-type Dashboard widgets, but stays in Net Worth and upcoming dues until balance reaches zero | 2.1/3.1 (v4) |
| Internal (own-account) transfers | New `Internal Transfer` spending category, excluded from Category Trend Detection | 2.4/3.11a (v4) |

...
## Product Specification v5 — BASELINE

**Status: Baselined.** This version is frozen as the reference for implementation (database schema, ER diagram, build work). Further changes should be made as a new version (v6) rather than edited in place, so this baseline stays a stable point to build against and compare future changes to. Supersedes v4.
...
Re-checked the spec against a simpler test: every dollar that moves must have somewhere it visibly goes, and the app must be able to answer "how much cash do I have," "what's due soon," "am I saving," and "where are my money pits" from its own data. Six gaps found:

1. **Source/Destination semantics didn't cover inbound-only money (2.2)**: `Source Account` was marked unconditionally Required, but Dividend/Income transactions have no internal account to debit — 2.5 called this "source is external" without reconciling it against 2.2's field rule. Clarified: `Source Account` always identifies the one internal account touched by a single-leg transaction, and `Indicator` (Debit/Credit) — not which field it's in — determines direction.
2. **Loan Disbursement had no double-entry mapping (2.5)**, and its relationship to `Account.Beginning Balance` was undefined. Added the disbursement leg and a rule for when to use Beginning Balance (pre-existing loans) vs. a Disbursement transaction (loans originated after go-live), so a new loan's debt is always traceable through transaction history rather than hand-set on the account record.
3. **`eCash Transfer` / `eCash Payment` (2.3) referenced an e-wallet account type that 2.1 explicitly defers to a future version** — these transaction types had no valid account to post to. Resolved: e-wallets are modeled as `Bank`-type Accounts for v1 (a dedicated e-wallet Account Type remains future work), so the transaction types stay usable now.
4. **`Bills` (2.3) was a dead, unreferenced enum value** distinct from `Bills Payment` — removed. `Bills Payment` is now explicitly defined as the mechanical type backing Utility (3.7) and Tax/Fee (3.9) payment transactions, so it has a clear purpose instead of floating undefined.
5. **"Am I saving money given my habits?" had no answer unless AI Analysis was opted in.** Savings rate is plain arithmetic over transactions the app already has — added a deterministic **Savings Rate** Dashboard widget (3.1) that works with AI fully disabled, on the same footing as Money-Pit detection (3.11a).
6. **Clarified how "underutilized subscription" money pits get surfaced.** 3.11a's automatic detector stays strictly fact-based (cost/frequency only — it has no usage data and never claims underutilization on its own). The **AI Analysis / AI Assist** layer (3.11) is where pattern-based *hypotheses* belong — e.g., "four similar recurring charges in one category, you may want to review whether you need all of them" — explicitly phrased as a suggestion to verify, not a factual usage claim, and only available when AI Analysis is enabled.
...
| Beginning Balance | For a **pre-existing** account being onboarded into ClearPath AI — its balance as of setup, with no matching transaction history behind it. **Loan/Credit Card v5 clarification**: a loan/card that already existed before using the app is seeded this way; a loan/card **originated after go-live** is instead set up with Beginning Balance = 0 and its opening amount recorded as a Loan Disbursement transaction (2.5) / first Card Purchase, so the debt is traceable through transaction history rather than hand-set on the record. |
...
**v5 fix**: `Bills` (the non-payment leftover from the original v1 type list) is removed — it was never referenced anywhere else in this spec. `Bills Payment` is retained and is the mechanical Transaction Type used for one-off Utility (3.7) and Tax/Fee (3.9) payment transactions.

**v5 fix**: `eCash Transfer` / `eCash Payment` (e.g., GCash, Maya) run against `Bank`-type Accounts in v1 — a dedicated e-wallet Account Type is future work (2.1), but the transaction types themselves are usable today since the unified model already supports them on a Bank account.
...
- Dividend received → credit investment or bank account (single-leg, per 2.2's field semantics — no separate external account record needed).
- Income received → credit the receiving account (single-leg, per 2.2's field semantics).
- Utility/tax payment → debit source (cash/bank), single-leg expense (per 2.2's field semantics).
...
- **Savings Rate (new v5)**: deterministic, no AI required — `(Income Received total − expense total) / Income Received total` over a selectable trailing period (e.g. 30/90 days), computed straight from Posted transactions. Always available regardless of whether AI Analysis (3.11) is enabled, the same way Money-Pit detection (3.11a) works without AI — answering "am I able to save money given my current habits" shouldn't require an opt-in external API call.
...
### 3.6 Credit Cards
- Set up card (Issuer Name, Description, Credit Limit, Card Balance, Interest Rate, Minimum Payment, Cut-off Date, Due Date, optional Reminder Lead Time override) — `Available Balance` is shown on the card's detail view but is derived (Credit Limit − Card Balance, 2.1/2.4b), not something the user sets up.
...
- Create income transactions (Transaction Type: Income Received, 2.3); list recent (within the month) transactions.
...
- **Inputs**: account balances, transaction history, spending categories, active goals, Money-Pit flags (3.11a).
- **Outputs**:
...
| Single-leg transaction field semantics | `Source Account` = the one internal account touched; `Indicator` (not field placement) determines debit/credit direction | 2.2 (v5) |
| Loan Disbursement | Two-leg like Loan Payment: credits destination (cash/bank), simultaneously establishes/increases Loan `Current Balance`. Beginning Balance is for pre-existing loans only; new loans use Disbursement | 2.1/2.5 (v5) |
| E-wallet account modeling | Modeled as `Bank`-type Accounts for v1; dedicated e-wallet Account Type deferred | 2.1/2.3 (v5) |
| Orphaned `Bills` transaction type | Removed; `Bills Payment` retained and defined as backing Utility/Tax one-off payments | 2.3 (v5) |
| Savings rate without AI | Added deterministic Dashboard Savings Rate widget, independent of AI Analysis opt-in | 3.1 (v5) |
| Subscription underutilization detection | 3.11a stays fact-only (cost/frequency); AI Assist (3.11) may suggest possible redundancy as a hypothesis, not a factual usage claim — no new usage-tracking input added | 3.11/3.11a (v5) |

...
## Product Specification v8 — BASELINE

**Status: Baselined.** This version is frozen as the reference for implementation (database schema, ER diagram, build work). Further changes should be made as a new version (v9) rather than edited in place, so this baseline stays a stable point to build against and compare future changes to. Supersedes v7.
...
`Goal` was the last entity in the spec still described purely in module prose, and one specific consequence of that fell into a bug pattern this project has already been burned by once:

1. **No goal completion trigger was defined anywhere.** §3.10 referred to "active goals" (also in §3.1, §3.11) and the Multi-debt allocation rolldown needed to know when a debt is "paid off," but nothing said what that meant. Since v4 deliberately decoupled Account `Status` from balance (a Closed account can still carry a balance; an Active one can be at $0), an implementation that checked *Status* instead of *balance* here would reproduce the exact bug flagged as the highest-value catch from the earlier (pre-scrap) finance-app review: a closed debt with money still owed looking "done." Fixed by defining completion strictly in terms of balance, and having the rolldown engine's "is this debt done" check reuse the same definition.
2. **`Goal` formalized as a data-model entity (new §2.8)**, matching how Account/Transaction/Settings/Recurring Rule are already specified — including a `Status` field (Active/Completed/Abandoned) and a proper Goal↔Account allocation join, instead of both being reverse-engineered from §3.10's prose at schema time.
...
### 2.8 Goal Model (new — was previously prose-only in 3.10)
...
| Status | Active, Completed, or Abandoned. Set automatically to Completed per the trigger rules below — never manually toggled, so it can't drift from what the linked accounts' actual balances say. Abandoned is user-initiated (e.g., after a linked account closes and the user chooses to retire the goal instead of re-linking, 2.1). |
...
**Completion trigger (v8 fix — previously undefined anywhere)**: defined strictly in terms of balance, **never** Account `Status` (2.1) — an account can be Closed while still owing money, or Active at $0, so Status alone would either mark an unpaid closed debt "complete" too early or leave a genuinely-finished goal stuck open.
- **Debt Payoff**: Status → Completed automatically the moment every linked Loan/Credit Card account's balance (`Current Balance` / `Card Balance`) reaches zero.
- **Emergency Fund / Savings Target / Investment Target**: Status → Completed automatically when the Allocation-adjusted sum of linked accounts' balances reaches Target Amount.
...
- **Completion**: automatic, defined in 2.8 — never a manual toggle, and never based on Account Status (2.1).
...
| Goal data model | Formalized as `Goal` + `Goal Account Link` entities, matching Account/Transaction/Settings/Recurring Rule | 2.8 (v8) |
| Goal completion trigger | Defined strictly by balance (linked debts at zero, or linked savings/investment balance at Target Amount) — never by Account Status, to avoid the closed-account-with-balance bug this project has already been burned by once. Multi-debt rolldown reuses the same "paid off" check | 2.8/3.10 (v8) |
